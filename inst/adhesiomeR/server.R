...
load("data/df_genes.rda")
load("data/df_systems.rda")
...
shinyServer(function(input, output, session) {
  ...
  analysis_results <- eventReactive(input[["blast"]], {
    req(input[["seq_file"]])
    if(length(input[["seq_file"]][["name"]]) > 20) {
      showModal(modalDialog(
        title = "Too many files!",
        "You can analyse up to 50 files at once using the GUI. For larger analyses please use
  ...
  observe({
    if(all(!is.na(analysis_results()[[1]][["Query"]]))) {
  ...
  output$ecor1 <- downloadHandler(
    filename="GCF_003333385.1.fna",  # desired file name on client
    content=function(con) {
      file.copy("data/GCF_003333385.1.fna", con)
    }
  )
  output$ecor2 <- downloadHandler(
    filename="GCF_003333545.1.fna",  # desired file name on client
    content=function(con) {
      file.copy("data/GCF_003333545.1.fna", con)
    }
  )
  output$ecor3 <- downloadHandler(
    filename="GCF_003333605.1.fna",  # desired file name on client
    content=function(con) {
      file.copy("data/GCF_003333605.1.fna", con)
    }
  )
